...
class Knuverse:
    ...
    def auth_refresh(self, apikey=None, secret=None, email=None, username=None, password=None):
        """
        ...
        :param email: Email or tenant_id to use for authentication
        :type apikey: str or None
        :param username: Username to use when using client login
        :type username: str or None
        :param apikey: Password corresponding to email
        :type apikey: str or None
        :Returns: None
        """
        jwt = self.auth_token(apikey=apikey, secret=secret, email=email, username=username, password=password)
        self._headers["Authorization"] = "Bearer %s" % jwt

        self._auth_token = jwt
        self._last_auth = datetime.utcnow()

    def auth_token(self, apikey=None, secret=None, email=None, username=None, password=None):
        """
        ...
        elif (email and password and not username) or (self._email and self._password and not self._username):
            body = {
                "user" : email or self._email,
                "password" : password or self._password
            }
        elif (email and password and username) or (self._email and self._password and self._username):
            body = {
                "account_number": username or self._username,
                "user" : email or self._email,
                "password" : password or self._password
            }
        else:
